...
from labelme.widgets import CreateVideoDialog
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        showdir = action(
            text=self.tr("Show Dir in Explorer"),
            slot=self.showDir,
            icon="open",
            tip=self.tr("Show Dir in Explorer"),
        )
        openNextImg = action(
            self.tr("&Next Image"),
            self.openNextImg,
        ...
        integerTech = action(
            "",
            slot=self.integerSite,
            shortcut=None,
            icon="IntegerTech",
            tip="Integer Technologies"
        )
        ...
        keepPrevContrast = action(
            self.tr("Keep Previous &Contrast"),
            self.enableKeepPrevContrast,
        ...
        self.actions = utils.struct(
            ...
            keepPrevContrast=keepPrevContrast,
            fitWindow=fitWindow,
            fitWidth=fitWidth,
        ...
        self.menus = utils.struct(
            ...
            recentDirs=QtWidgets.QMenu(self.tr("Open Recent &Directory")),
            labelList=labelMenu,
        )
        ...
        button.setIcon(QtGui.QIcon(QtGui.QPixmap(utils.qt.getIconStr("IntegerTech"))))
        button.setIconSize(QtCore.QSize(40, 37))
        button.clicked.connect(lambda: self.integerSite())

        # Wrap it in a QWidget with a layout so we can add margins
        container = QtWidgets.QWidget()
        layout = QtWidgets.QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)  # Left, Top, Right, Bottom
        layout.addWidget(button)

        # Add to toolbar as a QWidgetAction
        icon_action = QtWidgets.QWidgetAction(self)
        icon_action.setDefaultWidget(container)
    ...
    def integerSite(self):
        url = "https://www.integer-tech.com/"
        webbrowser.open(url)
    ...
    def updateFileMenu(self):
        ...
        menu.clear()
        dirs = [d for d in self.recentDirs if d != self.lastOpenDir and exists(d)]
        for i, d in enumerate(dirs):
            icon = utils.newIcon("open")
            action = QtWidgets.QAction(
                icon,
                "&%d %s" % (i + 1, d.split("/")[-1]),
                self
            )
            action.triggered.connect(functools.partial(self.importDirImages, d))
            menu.addAction(action)
        self.actions.showdir.setEnabled(bool(self.lastOpenDir))
    ...
    def enableKeepPrevBrightness(self, enabled):
        ...
        self.actions.keepPrevBrightness.setChecked(enabled)

    def enableKeepPrevContrast(self, enabled):
        self._config["keep_prev_contrast"] = enabled
        self.actions.keepPrevContrast.setChecked(enabled)
    ...
    def loadFile(self, filename=None):
        ...
        # zoom will only be reset if the progam loads an image for the first time or keep previous scale is off
        if is_initial_load or not self._config["keep_prev_scale"]:
            # if user had had previously zoomed into this image, retrieve those settings
            if self.filename in self.zoom_values:
                self.zoomMode = self.zoom_values[self.filename][0]
                self.setZoom(self.zoom_values[self.filename][1])
            # If no settings could be found, or the program is loading, reset zoom entirely
            elif is_initial_load:
                self.adjustScale(initial=True)
        
        # set scroll values
        for orientation in self.scroll_values:
            # If user had keep previous scale on, get the settings they had for the last image they had open and set the scroll values to that
            if self._config["keep_prev_scale"]:
                if self.recentFiles[0] in self.scroll_values[orientation]:
                    self.setScroll(
                        orientation, self.scroll_values[orientation][self.recentFiles[0]]
                    )
            # Else, retrieve scroll settings for the current image if they exist and apply them
            else:
        ...
        # Get the brightness and contrast of the currently loaded file if they exist
        brightness, contrast = self.brightnessContrast_values.get(
            self.filename, (None, None)
        )

        # If user has keep previous brightness, retrieve the brightness they had set for the last file they had open if it exists
        if self._config["keep_prev_brightness"] and self.recentFiles:
            brightness, _ = self.brightnessContrast_values.get(
                self.recentFiles[0], (None, None)
            )
        # If user has keep previous contrast, retrieve the contrast they had set for the last file they had open if it exists
        if self._config["keep_prev_contrast"] and self.recentFiles:
            _, contrast = self.brightnessContrast_values.get(
                self.recentFiles[0], (None, None)
            )
        
        # If a brightness and a contrast was found, apply them accordingly
        if brightness is not None:
            dialog.slider_brightness.setValue(brightness)
        if contrast is not None:
            dialog.slider_contrast.setValue(contrast)
        ...
        # update brightness / contrast dialog
        if brightness is not None or contrast is not None:
            dialog.onNewValue(None)
        self.paintCanvas()

        # Add loaded file as the most recent file
    ...
    def scanAllImages(self, folderPath):
        ...
        for file in os.listdir(folderPath):
            fullPath = osp.join(folderPath, file)
            if osp.isfile(fullPath) and file.lower().endswith(tuple(extensions)):
                images.append(os.path.normpath(fullPath))
